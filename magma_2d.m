function [A,G,Fp,lambda,z,rm,ind,VP,W2] = magma_2d(nr,R,nz,L,M,order)
%[A,G,Fp,lambda,z,rm,ind] = magma_2d(M,SAT)
...
truncate = 1;
[rp,rm,Pp,Pm,Qp,Qm] = sbp_injection(order,nr,dr,truncate);
...
[Pr_inv,Dr] = sbp_sparse(order,nr-1,dr); 
...
%Assemble first derivative for calculating velocity gradients
D1r =  kron(speye(nz+1),Dr);
...
if test_energy
    ...
    S = spdiags(S_diag,0,numel(A(:,1)),numel(A(:,1)));%spdiags(S_diag,0,nq,nq);
%     W_diag
keyboard
    E = S*A + A'*S;  
    lambda_E = eig(full(E));
