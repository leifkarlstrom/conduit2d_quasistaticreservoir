function [input_params, magmastatic_struct] = magma_prop_from_volatiles_variableA_noextra(conddip,...
...
%         CO2solwtp_table, H2O_gas_molfrac_table, Pvec(i)+deltaP);
    ...
    ddensity_dn_vec(i) = -density_vec(i)^2 .*(1./gas_density_vec(i) - 1./melt_density_vec(i)) ;
...
% for interpolating purposes
zvec_condlake_resized = [zvec(1:restop_ind-2);Hcolumn];
...
%% Conduit-Lake subsets
Pvec_CL = Pvec(1:restop_ind);
mu_vec_CL = mu_vec(1:restop_ind);
density_vec_CL = density_vec(1:restop_ind);
soundspeed_vec_CL = soundspeed_vec(1:restop_ind);
soundspeed_vec_cond = soundspeed_vec(Hlake_ind:restop_ind);
bulkmod_vec_CL = bulkmod_vec(1:restop_ind);
compress_vec_CL = ddensity_dP_vec(1:restop_ind);
CO2_vec_CL = CO2_vec(1:restop_ind);
H2O_vec_CL = H2O_vec(1:restop_ind);

dgasmassfrac_dP_vec_CL = dgasmassfrac_dP_vec(1:restop_ind);
ddensity_dn_vec_CL = ddensity_dn_vec(1:restop_ind);
...
% added output vector of three variables (mini output)
% mask = zvec <= 900;
% z900 = zvec(mask);
...
%host rock shear mod
G = 3.08e9; %Pa (taken from Crozier&Karlstrom 2022, Table S1)

% quick average tubewave speed calculation - Full Culumm
%Equ 28 from McQuillan&Karlstrom (2021)

tubewavespeed=sqrt(1./density_vec.*(1./bulkmod_vec+1/G).^(-1));
tubewavespeed_vec_CL = tubewavespeed(zvec<Hcolumn);
tubewavespeed_vec_cond = tubewavespeed(Hlake_ind:restop_ind);

%% save input parameters

input_params = struct();

input_params.conddip = conddip;
input_params.Rres = Rres;
input_params.Hcolumn = Hcolumn;
input_params.Hcond = Hcond;
input_params.conddip = conddip;
input_params.Rcondtop = Rcondtop;
input_params.Rcondbot = Rcondbot; 
input_params.Rlake = Rlake; 
input_params.TK_condbot = TK_condbot; 
input_params.TK_condtop = TK_condtop;
input_params.TK_lakebot = TK_lakebot; 
input_params.TK_laketop = TK_laketop; 
input_params.H2O_laketop_wtp = H2O_laketop_wtp; 
input_params.H2O_lakebot_wtp = H2O_lakebot_wtp; 
input_params.H2O_condtop_wtp = H2O_condtop_wtp; 
input_params.H2O_condbot_wtp = H2O_condbot_wtp; 
input_params.CO2_laketop_ppm = CO2_laketop_ppm; 
input_params.CO2_lakebot_ppm = CO2_lakebot_ppm;
input_params.CO2_condtop_ppm = CO2_condtop_ppm;
input_params.CO2_condbot_ppm = CO2_condbot_ppm;
input_params.G = G;

% flip all profiles (bottom is top of lake) for reduced order model input

bulkmod_vec_flip = flip(bulkmod_vec);
bulkmod_vec_CL_flip = flip(bulkmod_vec_CL);
soundspeed_vec_flip = flip(soundspeed_vec);
soundspeed_vec_CL_flip = flip(soundspeed_vec_CL);
soundspeed_vec_cond_flip = flip(soundspeed_vec_cond);
density_vec_flip = flip(density_vec);
density_vec_CL_flip = flip(density_vec_CL);
mu_vec_flip = flip(mu_vec);
mu_vec_CL_flip = flip(mu_vec_CL);
Pvec_flip = flip(Pvec);
Pvec_CL_flip = flip(Pvec_CL);
zvec_flip = flip(zvec);
zvec_condlake_flip = flip(zvec_condlake);
zvec_lake_flip = flip(zvec_lake);
compress_vec_CL_flip = flip(compress_vec_CL);
CO2_vec_CL_flip = flip(CO2_vec_CL);
H2O_vec_CL_flip = flip(H2O_vec_CL);
tubewavespeed_vec_CL_flip = flip(tubewavespeed_vec_CL);
tubewavespeed_vec_cond_flip = flip(tubewavespeed_vec_cond);
dgasmassfrac_dP_vec_flip = flip(dgasmassfrac_dP_vec_CL);
ddensity_dn_vec_flip = flip(ddensity_dn_vec_CL);

% volatiles
volatiles_CL = (CO2_vec_CL_flip + H2O_vec_CL_flip);
delta_volatiles_CL= abs(volatiles_CL(end) - volatiles_CL(1));



%% %% evaluate organ-pipe frequency based on Eq 29 and 30
%from McQuillan & Karlstrom (2021)
%use depth-avergaged values from above magmastatic code

n = 1;
m = 1;

% CONDUIT ONLY
%calculate organ pipe resonant frequency and period: open/open system
%Equ 29 from McQuillan&Karlstrom (2021)

ct = mean(tubewavespeed_vec_cond);

freq_openopen = (n*ct) / (2*Hcond);
T_openopen = 1 / freq_openopen;

%calculate organ pipe resonant frequency and period: open/closed system
%Equ 30 from McQuillan&Karlstrom (2021)

f_openclosed = (m*ct) / (4*Hcond);
T_openclosed = 1 / f_openclosed;


%% save profile vectors

magmastatic_struct = struct();

% flipped for CR RO model input 

magmastatic_struct.bulkmod_vec = bulkmod_vec_flip;
magmastatic_struct.bulkmod_vec_CL = bulkmod_vec_CL_flip;
magmastatic_struct.soundspeed_vec = soundspeed_vec_flip;
magmastatic_struct.soundspeed_vec_CL = soundspeed_vec_CL_flip;
magmastatic_struct.soundspeed_vec_cond = soundspeed_vec_cond_flip;
magmastatic_struct.density_vec = density_vec_flip;
magmastatic_struct.density_vec_CL = density_vec_CL_flip;
magmastatic_struct.mu_vec = mu_vec_flip;
magmastatic_struct.mu_vec_CL = mu_vec_CL_flip;
magmastatic_struct.Pvec = Pvec_flip;
magmastatic_struct.Pvec_CL = Pvec_CL_flip;
magmastatic_struct.zvec = zvec_flip;
magmastatic_struct.zvec_condlake = zvec_condlake_flip;
magmastatic_struct.zvec_lake = zvec_lake_flip;
magmastatic_struct.compress_vec_CL = compress_vec_CL_flip;
magmastatic_struct.CO2_vec_CL = CO2_vec_CL_flip;
magmastatic_struct.H2O_vec_CL = H2O_vec_CL_flip;
magmastatic_struct.volatiles_CL = volatiles_CL;
magmastatic_struct.delta_volatiles_CL= delta_volatiles_CL;
magmastatic_struct.tubewavespeed_vec_CL = tubewavespeed_vec_CL_flip;
magmastatic_struct.tubewavespeed_vec_cond = tubewavespeed_vec_cond_flip;
magmastatic_struct.T_openopen = T_openopen;
magmastatic_struct.T_openclosed = T_openclosed;
magmastatic_struct.dgasmassfrac_dP_vec=dgasmassfrac_dP_vec_flip;
magmastatic_struct.ddensity_dn_vec = ddensity_dn_vec_flip;

%unflipped
% magmastatic_struct.conduit_top_ind = Hlake_ind; % this is an indices for conduit top in 'zvec'
% magmastatic_struct.conduit_bot_ind = restop_ind; % this is an indices for conduit bottom in 'zvec'
% magmastatic_struct.res_bot_ind = resbot_ind; % this is an indices for reservoir bottom in 'zvec'
% magmastatic_struct.yticklist = yticklist; % this gives a y-axis in meters
% magmastatic_struct.ylimits = ylimits; % this creates a ylim at the bottom of the conduit (so just the lake an conduit are plotted when plotting 'zvec' vectors)
