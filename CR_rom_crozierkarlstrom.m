...
Pm.drhodz = gradient(Pm.rho,out.M.dz); %density gradient 
Pm.mu = out.M.mu*ones(size(Pm.rho)); %viscosity as fctn of depth z in Pas

%parameter definitions for conduit (need to be consistent with simulation!)
Pm.g = out.M.g; %gravity

Pm.rad = out.M.R; %conduit radius as fct of z

Pm.Ct = out.Ct;%total storativity of reservoir (magma+elastic)
...
% Tinvis = 2*pi*sqrt(Pm.H*0.5*(Pm.rho(1)+Pm.rho(end))/ ...
%     ((Pm.rho(end)-DeltaRho)*Pm.g+pi*Pm.rad(1)^2/Pm.Ct));

%fully developed resonant period
Tfd = 2*pi./sqrt(((Pm.rho(end)-DeltaRho)*Pm.g+pi*Pm.rad(1)^2/Pm.Ct)./(Pm.H*mean(Pm.rho)) - ...
...
Qfd = Pm.rad(1)^2 * mean(Pm.rho)/(8*mean(Pm.mu)) * ...
    sqrt((Pm.g*(Pm.rho(end)-DeltaRho)+ pi*Pm.rad(1)^2/Pm.Ct)/(Pm.H*mean(Pm.rho)) - ...
    16*mean(Pm.mu)^2/(Pm.rad(1)^4*mean(Pm.rho)^2));
